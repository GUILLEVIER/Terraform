...
provider "aws" {
  region = local.region
  access_key = "AKIASGQK25OB36AXBJXN"
  secret_key = "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"
}
...
# Instancia EC2
# Usar _ para el nombre de los recursos
# ------------------------
...
resource "aws_instance" "servidor_1" {
  ...
  # Asocia el segurity group a la instancia
  vpc_security_group_ids = [aws_security_group.mi_grupo_de_seguridad.id]
  # Se agrega el ID de la subnet a la cual queremos desplegar nuestra instancia
...
# De esta forma ya nadie más podria acceder directamente
...
resource "aws_lb_target_group" "this" {
  ...
  health_check {
    ...
    # caso contrario la instancia esta rota o caida.
    matcher = "200"
    path    = "/"
